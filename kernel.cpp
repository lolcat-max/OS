...
void *memcpy(void* d, const void* s, size_t n) { uint8_t* dd=d; const uint8_t* ss=s; for(size_t i=0;i<n;i++) dd[i]=ss[i]; return d; }
void *memset(void* d, int v, size_t n) { uint8_t* dd=d; for(size_t i=0;i<n;i++) dd[i]=(uint8_t)v; return d; }
size_t strlen(const char *s) { size_t i=0; while(s && s[i]) i++; return i; }
int strcmp(const char*a,const char*b){ while(*a&&*a==*b){a++;b++;} return *(unsigned char*)a-*(unsigned char*)b; }
void *malloc(size_t s);  // use your kernel’s allocator
void free(void *p);
...
void TerminalWindow::handle_command() {
    ...
    else if (strncmp(current_line, "compile ", 8) == 0) {
		const char* fname = current_line + 8;
		char* src = fat32_read_file_as_string(fname);
		if (!src) { console_print("File not found.\n"); return; }
		extern "C" int tcc_compile_and_run(const char* src);
		int rc = tcc_compile_and_run(src);
		char buf[64];
		snprintf(buf, sizeof(buf), "Program exited with code %d\n", rc);
		console_print(buf);
	}
