...
struct Elf32_Ehdr {
    uint32_t e_magic; uint8_t e_class, e_data, e_version, e_osabi, e_abiver, e_pad[7];
    uint16_t e_type, e_machine; uint32_t e_version2, e_entry, e_phoff, e_shoff, e_flags;
...
bool load_busybox_native(const char* path, Window* win) {
    char* data = fat32_read_file_as_string(path);
    if (!data) return false;
    Elf32_Ehdr* eh = (Elf32_Ehdr*)data;
    if (eh->e_magic != ELF_MAGIC) { delete[] data; return false; }
    ...
    win->console_print("BusyBox Loaded. Entering Entry Point...\n");
    // Call entry point - requires syscall stubs for write/read
    return true;
}
